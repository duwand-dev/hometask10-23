...
import { GetTimeSlot } from "../../Actions/TimeSlotAction";
import { AssignInterview, GetInterview } from "../../Actions/InterviewAction";

const interviewEmail = "PYJ@inter.com"
...
export const UserPanel = () => {
  ...
  useEffect(() => {
    GetTimeSlot(interviewEmail).then((res) => {
      setTimeSlots(res);
      ...
      GetInterview(localStorage.getItem("email") as string, "User").then(
        (res) => {
          const interviews = res.rows;
          for (let i = 0; i < interviews.length; i++) {
            selections[interviews[i].startt][interviews[i].date % 7 + 1] = 1;
            titles[interviews[i].startt][interviews[i].date % 7 + 1] = interviews[i].title;
          }
  ...
  const handleMouseUp = (
    ...
    if (selections[row][col] == 2) {
      let title = prompt("Input your interview title!");
      if (title) {
    ...
    } else alert("The interview is not available at that point!");
    setStartSel({ row: -1, col: -1 });
  };
                ...
                {days.map((day: string, index1: number) => (
                  <td
                    style={{
                      ...
                      textAlign: "center",
                      flex: 1,
                      color: (selections && selections[index] && selections[index][index1] == 1 ? "white" : "black"),
                      background: (
                  ...
                  >
                    {index == 0
                      ...
                      : (titles && titles[index] ? titles[index][index1] : "")}
                  </td>
                ))}
              </tr>
            ))}
